...
library(janitor)
library(kableExtra)
tk_pns <- read_excel("G:/My Drive/# REGISTRASI DAN STATISTIK/Website/212202/static/SDM_tendik_pns.xlsx")
tk_non_pns <- read_excel("G:/My Drive/# REGISTRASI DAN STATISTIK/Website/212202/static/SDM_tendik_non_pns.xlsx")
...
> Sebaran tendik PNS Universitas Sultan Ageng Tirtayasa berdasarkan tingkat pendidikan dan jenis kelamin.

```{r, echo=FALSE, out.width="60%"}
data <- tk_pns %>%
  group_by(PENDIDIKAN, JK) %>%
  summarize(banyak = n(), .groups = "drop") %>%
  pivot_wider(names_from = JK, values_from = banyak)
data$PENDIDIKAN <- c("S2", "S1", "Diploma", "SLTA", "SLTP", "SD")
...
names(data) <- c("Golongan", "Laki-Laki", "Perempuan")
...
> Sebaran tendik non PNS Universitas Sultan Ageng Tirtayasa berdasarkan tingkat pendidikan dan jenis kelamin.
